      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="脱敏字段规则配置" prop="rules">
          ...
          >
            ...
            <el-table-column label="脱敏类型" align="center">
              <template slot-scope="scope">
                <dict-tag :options="dict.type.data_cipher_type" :value="scope.row.cipherType"/>
              </template>
            </el-table-column>
            <el-table-column label="规则类型" align="center">
              <template slot-scope="scope">
                <dict-tag :options="dict.type.data_regex_crypto" :value="scope.row.cryptType"/>
              </template>
            </el-table-column>
          </el-table>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select v-model="form.status" disabled>
            <el-option
              v-for="dict in dict.type.data_cipher_status"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
      ...
      <el-dialog :title="cipher.title" :visible.sync="cipher.open" width="400px" append-to-body>
        <el-form ref="form2" :model="form2" :rules="rules2" label-width="80px">
         ...
         <el-form-item label="脱敏类型" prop="cipherType">
           <el-select v-model="form2.cipherType" clearable placeholder="请选择脱敏类型" >
             <el-option
               v-for="dict in dict.type.data_cipher_type"
               :key="dict.value"
               :label="dict.label"
               :value="dict.value"
             />
         ...
         <el-form-item label="规则类型" prop="cryptType">
           <el-select v-model="form2.cryptType" clearable placeholder="请选择规则类型" v-if="form2.cipherType == 1">
             <el-option
               v-for="dict in dict.type.data_regex_crypto"
               :key="dict.value"
               :label="dict.label"
               :value="dict.value"
             />
           </el-select>
           <el-select v-model="form2.cryptType" clearable placeholder="请选择规则类型" v-if="form2.cipherType == 2">
             <el-option
               v-for="dict in dict.type.data_algorithm_crypto"
               :key="dict.value"
               :label="dict.label"
               :value="dict.value"
             />
           </el-select>
           <el-select v-model="form2.cryptType" clearable placeholder="请选择规则类型" v-if="form2.cipherType == undefined">
           </el-select>
         </el-form-item>
        </el-form>
...
export default {
  name: 'ApiMaskEdit',
  dicts:['data_cipher_type', 'data_regex_crypto', 'data_algorithm_crypto','data_cipher_status'],
  props: {
    data: {
      type: Object,
      default: function() {
        ...
        if (dataApi && dataApi.resParams.length > 0) {
          ...
          this.form.rules.forEach(rule => {
            ...
            if (fieldParamIndex !== -1) {
              const cipher = rule.cipherType
              let crypt = {}
              if (rule.cipherType === '1') {
                crypt = rule.cryptType
              } else if (rule.cipherType === '2') {
                crypt = rule.cryptType
              }
              const resParam = Object.assign({}, this.resParamList[fieldParamIndex], { cipherType: (cipher) || undefined, cryptType: (crypt) || undefined })
