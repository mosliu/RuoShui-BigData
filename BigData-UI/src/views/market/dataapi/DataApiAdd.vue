    ...
    <div class="body-wrapper">
      ...
      <el-form v-if="active == 1" ref="form1" :model="form1" :rules="rules1" label-width="80px">
        ...
        <el-form-item label="API路径" prop="apiUrl">
          <el-input v-model="form1.apiUrl"  disabled />
        </el-form-item>
        ...
        <el-form-item label="返回格式" prop="resType">
          <el-select v-model="form1.resType" placeholder="请选择返回格式">
            <el-option
              v-for="dict in dict.type.sys_api_return"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
        ...
        <el-form-item label="是否限流" prop="rateLimit">
          <el-select v-model="form1.rateLimit.enable">
            <el-option
              v-for="dict in dict.type.sys_api_limit"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
        ...
        <el-form-item label="状态" prop="status">
          <el-select v-model="form1.status" disabled>
            <el-option
              v-for="dict in dict.type.sys_api_status"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
      ...
      <el-form v-if="active == 2" ref="form2" :model="form2" :rules="rules2" label-width="80px">
        <el-form-item label="配置方式" prop="configType">
          <el-select v-model="form2.configType" placeholder="请选择配置方式" @change="configTypeSelectChanged">
            <el-option
              v-for="dict in dict.type.sys_api_config_type"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
            ></el-option>
         ...
         <el-table-column prop="paramType" label="参数类型" align="center" show-overflow-tooltip>
           <template slot-scope="scope">
               <el-select  v-model="scope.row.paramType" placeholder="请选择参数类型">
                 <el-option
                   v-for="dict in dict.type.data_param_type"
                   :key="dict.value"
                   :label="dict.label"
                   :value="dict.value"
                 ></el-option>
...
export default {
  name: 'DataApiAdd',
     dicts: ['sys_api_status','sys_api_mode','sys_api_return','sys_api_limit','sys_api_config_type','data_where_type','data_param_type'],
  components: {
    SqlEditor
  },
  props: {
