<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-circle-plus" @click="handleCreate" v-hasPermi="['datax:job:add']">
        添加
      </el-button>
        ...
        <template slot-scope="scope"> {{ statusList.find(t => t.value === scope.row.lastHandleCode).label }}</template>
            ...
            <template slot-scope="scope">
                <el-button
                  ...
                  @click="handlerExecute(scope.row)"
                  v-hasPermi="['datax:job:trigger']"
                >执行一次</el-button>
                <el-button
                  size="mini"
                  type="text"
                  icon="el-icon-search"
                  @click="handlerViewLog(scope.row)"
                  v-hasPermi="['datax:job:selectlog']"
                >查询日志</el-button>
                <el-button
                  size="mini"
                  type="text"
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="1000px" :before-close="handleClose">
      ...
      <jsonEditor v-if="temp.glueType==='BEAN'" ref="jsonEditor" v-model="temp.jobJson" />
...
export default {
  ...
  methods: {
    ...
    // 查看日志
    handlerViewLog(row) {
      this.$router.push({ path: '/sjjc/joblog', query: { jobId: row.id }})
    },
    handlerStart(row) {
