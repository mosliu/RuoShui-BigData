...
import com.ruoshui.flink.mapper.JobConfigMapper;
...
import com.ruoshui.flink.streaming.web.model.entity.BatchJob;
...
public class JobConfigServiceImpl implements JobConfigService {
    ...
    public JobConfig getJobConfigByJobId(String jobid) {
        ...
        return jobConfigMapper.getJobConfigByJobId(jobid);
    }
     ...
     * @version V1.0
     * @see com.ruoshui.flink.service.JobConfigService#getJobConfigByIdContainDelete(Long)
     */
    @Override
    ...
    public void openOrClose(Long id, YN yn, String userName) {
        ...
        jobConfigMapper.updateByPrimaryKeySelective(jobConfig);
        if(jobConfig.getIsOpen() == 1){
            //加入调度
            BatchJob batchjob = jobConfigMapper.getAllBatchJobsById(id);
            if(null != batchjob){
                batchJobManagerScheduler.registerJob(batchjob);
            }
        }else if(jobConfig.getIsOpen() == 0){
            batchJobManagerScheduler.deleteJob(id);
        }
    }
    ...
    @Override
    public BatchJob getAllBatchJobsById(Long id) {
        return jobConfigMapper.getAllBatchJobsById(id);
    }
