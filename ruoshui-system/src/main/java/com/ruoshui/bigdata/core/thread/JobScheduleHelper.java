...
public class JobScheduleHelper {
    ...
    public void start() {
        ...
        scheduleThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                logger.info(">>>>>>>>> init ruoshui-ground admin scheduler success.");
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        ...
                        if (scheduleList != null && scheduleList.size() > 0) {
                            ...
                            for (JobInfo jobInfo : scheduleList) {
                                ...
                                if (nowTime > jobInfo.getTriggerNextTime() + PRE_READ_MS) {
                                    ...
                                    logger.warn(">>>>>>>>>>> ruoshui-ground, schedule misfire, jobId = " + jobInfo.getId());
                                    // fresh next
                                    ...
                                    logger.debug(">>>>>>>>>>> ruoshui-ground, schedule push trigger : jobId = " + jobInfo.getId());
                        ...
                        if (!scheduleThreadToStop) {
                            logger.error(">>>>>>>>>>> ruoshui-ground, JobScheduleHelper#scheduleThread error:{}", e);
                        }
                    } finally {
                ...
                logger.info(">>>>>>>>>>> ruoshui-ground, JobScheduleHelper#scheduleThread stop");
            }
        });
        scheduleThread.setDaemon(true);
        scheduleThread.setName("ruoshui-ground, admin JobScheduleHelper#scheduleThread");
        ...
        ringThread = new Thread(() -> {
            ...
            while (!ringThreadToStop) {
                ...
                try {
                    ...
                    // ring trigger
                    logger.debug(">>>>>>>>>>> ruoshui-ground, time-ring beat : " + nowSecond + " = " + Arrays.asList(ringItemData));
                    ...
                    if (!ringThreadToStop) {
                        logger.error(">>>>>>>>>>> ruoshui-ground, JobScheduleHelper#ringThread error:{}", e);
                    }
                }
            ...
            logger.info(">>>>>>>>>>> ruoshui-ground, JobScheduleHelper#ringThread stop");
        });
        ringThread.setDaemon(true);
        ringThread.setName("ruoshui-ground, admin JobScheduleHelper#ringThread");
        ringThread.start();
    }
    ...
    private void pushTimeRing(int ringSecond, int jobId) {
        ...
        logger.debug(">>>>>>>>>>> ruoshui-ground, schedule push time-ring : " + ringSecond + " = " + Arrays.asList(ringItemData));
    }
    ...
    public void toStop() {
        ...
        logger.info(">>>>>>>>>>> ruoshui-ground, JobScheduleHelper stop");
    }
