		...
		registryThread = new Thread(() -> {
			while (!toStop) {
				try {
					...
					if (!toStop) {
						logger.error(">>>>>>>>>>> ruoshui-ground, job registry monitor thread error:{}", e);
					}
				}
				try {
			...
			logger.info(">>>>>>>>>>> ruoshui-ground, job registry monitor thread stop");
		});
		registryThread.setDaemon(true);
		registryThread.setName("ruoshui-ground, admin JobRegistryMonitorHelper");
